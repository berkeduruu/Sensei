...
public class SenseiController : MonoBehaviour
{
    ...
    [Header("Düşme Kontrolü")]
    public float deathYThreshold = -10f; // Bu Y değerinin altına düşerse ölür
    ...
    private bool hasFallen = false; // Düşme kontrolü için flag
    ...
    void Update()
    {
        ...
        FlipCharacter();
        HandleFootstepSound();
        CheckFallDeath(); // Haritadan düşme kontrolü
    }
    ...
    {
        if (isDead || hasFallen) return;

        // Eğer karakter belirlenen Y eşiğinin altına düşerse öl
        if (transform.position.y < deathYThreshold)
        {
            hasFallen = true;
            Debug.Log($"⚠️ Karakter haritadan düştü! Y pozisyonu: {transform.position.y}, Eşik: {deathYThreshold}");
            
            // Direkt ölüm fonksiyonunu çağır (event sistemine güvenmek yerine)
            OnPlayerDeath();
            
            // Ayrıca HealthSystem'ı da güncelle (eğer varsa)
    ...
    public void OnPlayerDeath()
    {
        if (isDead) return;
        isDead = true;
        hasFallen = true; // Düşme flag'ini de set et
        
        Debug.Log("💀 OnPlayerDeath çağrıldı!");
        ...
        if (gameOverPanel != null)
        {
            ...
            Debug.Log("✅ Game Over paneli açıldı!");
        }
        else
        {
            Debug.LogError("❌ Game Over paneli atanmamış! Inspector'da Game Over Panel referansını ekle!");
        }
